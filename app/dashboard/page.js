...
import { Users, DollarSign, Calendar, PlusCircle, ArrowRight, TrendingUp, Activity } from 'lucide-react';
                  ...
                  <form onSubmit={handleNewDealSubmit} className="mb-4">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      ...
                      >
                        ...
                        <option value="Negotiation">Negoti
ation</option>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 mb-8">
            <div className="bg-white shadow rounded-lg">
              <div className="px-4 py-5 sm:p-6">
                <h2 className="text-lg font-medium text-gray-900 mb-4">Deals by Stage</h2>
                <div className="space-y-4">
                  {Object.entries(dealsByStage).map(([stage, count]) => (
                    <div key={stage} className="flex items-center">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStageColor(stage)} mr-2`}>
                        {stage}
                      </span>
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div
                          className="bg-indigo-600 h-2 rounded-full"
                          style={{ width: `${(count / Object.values(dealsByStage).reduce((a, b) => a + b, 0)) * 100}%` }}
                        ></div>
                      </div>
